...
### 3. 성능 최적화 — CSV 업로드 파이프라인 배치화

동일 데이터셋(daiso 상품 12,090개) 기준, 개선 전후를 계측 로그로 직접 비교했습니다. 개선 전은 초기 계측 단계라 "CSV 파싱+DB 저장", "임베딩+색인" 2개 구간으로만 로깅했고, 병목 구간(임베딩+색인)을 특정한 뒤 개선을 적용하면서 5단계로 세분화해 다시 계측했습니다.
...
계측 결과 **전체의 약 81%가 임베딩 + 색인 구간**에 집중되어 있었습니다. 원인은 GPU 연산 자체가 아니라 호출 오버헤드였습니다 — 상품 1건마다 AI 서버 임베딩 요청 1회, OpenSearch 색인 요청 1회가 발생해 12,090건 기준 왕복이 24,000회를 넘었습니다.

**개선 내용**

| 구간 | 적용한 방식 |
|------|-----------|
| 임베딩 | 100건씩 묶어 `/embed` 1회 호출 (12,090회 → 121회) |
| 색인 | 건별 index → `client.bulk()` 100건 단위 |
| DB 저장 | `saveAllAndFlush` + `hibernate.jdbc.batch_size: 100`, `order_inserts: true` |
| 재색인 대상 | `externalProductId` 기준 diff로 변경/신규 건만 임베딩 |
...
**약 52.3% 단축.** DB 구간은 385초 → 39초로 약 90% 줄었습니다. 남은 시간의 약 95.8%는 임베딩 + 색인 구간인데, 호출 오버헤드를 제거한 뒤라 대부분이 GPU 임베딩 연산 자체입니다.

> 이 개선은 요청을 배치로 묶은 것이며, 멀티스레드 병렬 처리는 적용하지 않았습니다. 업로드는 여전히 단일 요청 안에서 동기적으로 처리됩니다 — 아래 "알려진 이슈" 참고.
...
- [x] 업로드 소요시간 계측으로 병목(임베딩+색인) 특정 후 배치화 적용 — 타임아웃 해결, 전체 처리시간 약 52% 단축 (33분 59초 → 16분 13초)
...
| ~~`SecurityConfig`가 현재 전체 요청 permitAll~~ | 해결 | 엔드포인트별 인가 규칙 적용. 공개 API(가입/로그인/`/api/chat`/위젯) 외에는 인증 필요 |
| ~~CORS가 모든 오리진 허용(`http://**`) + `allowCredentials(true)`~~ | 해결 | 경로별로 분리 — 세션 쿠키를 쓰는 콘솔 API는 허용 오리진 명시, 쿠키를 쓰지 않는 `/api/chat`만 전체 허용 |
| ~~`/api/admin/**`에 인가 검사 없음~~ | 해결 | `hasRole("ADMIN")` 적용. 세션 속성을 기준으로 매 요청 권한을 재확인 |
| 위젯 무단 사용(타사 `clientCode` 도용 호출) 방어 없음 | 개선 예정 | CORS는 브라우저에만 적용되어 방어가 되지 않음 — 회사별 허용 도메인 검증 + 호출 빈도 제한 필요 |
| ~~기본 관리자 비밀번호가 `1234`로 하드코딩~~ | 해결 | `AdminInitializer`가 `ADMIN_EMAIL`/`ADMIN_PASSWORD` 환경변수를 사용. 비밀번호 미설정 시 계정을 생성하지 않음 |
| DB 트랜잭션과 OpenSearch 색인의 롤백 경계 불일치 | 개선 예정 | 색인 중 실패 시 DB는 롤백되나 색인된 문서는 잔존 → `AFTER_COMMIT` 이벤트 분리 + 재시도 검토 |
| 대용량 업로드가 동기 요청 내에서 처리(약 16분 대기) | 개선 예정 | 배치화로 시간은 줄였으나 구조적 해결은 아님 — 비동기 처리 + 진행률 조회 필요 |
| ~~기본 관리자 계정이 코드에 하드코딩(`AdminInitializer`)~~ | 해결 | `.env`의 `ADMIN_EMAIL`/`ADMIN_PASSWORD`로 주입 |
...
| 성능 트러블슈팅 | 계측 로그를 근거로 대용량 업로드의 병목 구간(임베딩+색인)을 특정하고, 건별 호출을 배치 호출로 전환해 전체 처리 시간을 약 52% 단축 |
...
| 홍채원 | 관리자 콘솔·위젯 프론트엔드(JS), 회원가입·승인 플로우 설계, CSV 배치 적재 파이프라인 구현(처리 시간 약 52% 단축), 쇼핑몰 데모 페이지 연동(66girls·Daiso), 발표 자료(PPT·화면정의서) 제작