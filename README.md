...
[Spring Boot Backend] :8080  (com.widgetrag.backend)
  ...
  └─ widget       — 임베드 스크립트 태그 발급 API
    ...
    │            product_items 인덱스, chunk_vector(dim=1024, HNSW, cosine)
...
### 6. 회사/직원 승인 워크플로우
쇼핑몰 운영사는 가입 후 플랫폼 관리자의 승인을 거쳐야 `client_code`가 발급되며, 회사 내 직원 추가도 회사 관리자의 승인을 거치는 2단계 권한 체계를 구현했습니다.
...
| 엔티티 | 설명 |
|------|------|
| `Company` | 쇼핑몰 운영사, `client_code`, 승인 상태(PENDING/APPROVED/REJECTED) |
...
- [x] 업로드 소요시간 계측을 통한 임베딩 배치 타임아웃 이슈 진단 및 해결
- [x] Toast/Modal 기반 UI 피드백 시스템으로 네이티브 alert/confirm 대체
...
| 성능 트러블슈팅 | 계측 로그를 근거로 대용량 업로드의 병목 구간(임베딩 배치 타임아웃)을 특정하고 타임아웃 값 조정으로 해결 |
