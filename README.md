...
동일 데이터셋(66girls 상품) 기준, 순차 처리와 병렬 처리를 계측 로그로 직접 비교했습니다. 순차 처리는 초기 계측 단계라 "CSV 파싱+DB 저장", "임베딩+색인" 2개 구간으로만 로깅했고, 병목 구간(임베딩+색인)을 특정한 뒤 병렬화를 적용하면서 5단계로 세분화해 다시 계측했습니다.
